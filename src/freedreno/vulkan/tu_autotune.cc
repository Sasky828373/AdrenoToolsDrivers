...
tu_autotune::get_env_config()
{
   ...
   std::call_once(once, [&] {
      ...
      /* A8xx adaptive performance default:
       * Use real GPU timing feedback for recurring render passes.
       * Explicit TU_AUTOTUNE_ALGO/drirc configuration always wins.
       */
      if (algo_strv.empty() &&
          device->physical_device->info->chip >= A8XX) {
      ...
      void update(rp_history &history, bool immediate)
      {
         ...
         if (immediate) {
            ...
            if (sysmem_ema.count < MIN_PROFILE_DURATION_COUNT || gmem_ema.count < MIN_PROFILE_DURATION_COUNT) {
               ...
               /* A8xx: converge faster once timing measurements consistently
                ...
                */
               /* A8xx GTA5 adaptive fast-lock.
                *
                * Keep profiling both modes first, but converge significantly
                * faster for stable recurring render passes. Decisions remain
